...
public class AudioController {
    ...
    public void addAudio(int index, AudioBean bean) {
        ...
        if (query <= -1) {
            ...
            if (!currentBean.getId().equals(bean.getId())) {
    ...
    public void changeFavourite() {
        ExecutorK.INSTANCE.execute(TAG, () -> {
            if (BizDb.INSTANCE.hasFavouriteBean(getNowPlaying().getId())) {
                //已收藏，移除
                BizDb.INSTANCE.removeFavouriteBean(getNowPlaying().getId());
                //GreenDaoHelper.removeFavourite(getNowPlaying());
                EventBus.getDefault().post(new AudioFavouriteEvent(false));
            } else {
                //未收藏，添加收藏
                BizDb.INSTANCE.addFavouriteBean(getNowPlaying());
